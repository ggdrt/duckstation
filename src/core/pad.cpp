  ...
  switch (m_active_device)
  {
    case ActiveDevice::None:
    {
      if (m_multitaps[m_JOY_CTRL.SLOT].IsEnabled())
      {
        if ((ack = m_multitaps[m_JOY_CTRL.SLOT].Transfer(data_out, &data_in)) == true)
        {
          ...
          Log_ErrorPrintf("Active device set to tap %d, sent 0x%02X to tap, received 0x%02X", static_cast<int>(m_JOY_CTRL.SLOT), data_out, data_in);
        }
      }
      else
      {
    ...
    case ActiveDevice::Multitap:
    {
      if (m_multitaps[m_JOY_CTRL.SLOT].IsEnabled())
      {
        ...
        Log_WarningPrintf("Transfer tap %d, send 0x%02X, receive 0x%02X, acked: %s", static_cast<int>(m_JOY_CTRL.SLOT), data_out, data_in, ack ? "true" : "false");
      }
    }
    break;
  }
