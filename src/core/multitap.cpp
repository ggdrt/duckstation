  ...
  switch (m_transfer_state)
  {
      ...
      switch (data_in)
      {
        ...
        case 0x04:
        {
          ...
          if (ack)
          {
            ...
            // Send access byte to remaining controllers if transferring all controllers
    ...
    case TransferState::ControllerCommand:
    {
      if (m_controller_transfer_step == 0) // Command byte
      {
        if (m_transfer_all_controllers)
        {
          // Unknown if 0x42 is the only valid command byte here, but other tested command bytes cause an early abort
          // after the request/status byte transfer
    ...
    case TransferState::AllControllers:
    {
      ...
      // The hardware is likely either latching the controller info halfword count or waiting for a transfer timeout
      // (timeouts are a possibility due to buffered responses in this mode). We'll pick the simpler option and just
      // check the ack return value since we know the behavior of our controller implementations.
